...
nodeio.context(
    name="Analysis_RSpec",
    description="finds rough readout freq",
...
# set inputs data for dry-run of the node
with SlabFile(r'dummy_data/res_spec.h5', 'r') as f:
...
while nodeio.status.active:
    ...
    f_r = fit_result['f']
    print('\n IF= %f +- %f MHz' % (f_r[0], f_r[1]))

    # parameters used to calculate fit equation
    kc = fit_result['kc'][0]
    k = fit_result['k'][0]
    offset = fit_result['offset'][0]
    slope = fit_result['slope'][0]
    y_fit = (offset)-((kc/k) / (1 + (4 * ((freqs - f_r[0]) ** 2) / (k ** 2)))) + slope * freqs

    fig = go.Figure()
    fig.update_xaxes(title_text='Freq (MHz)')
    fig.update_yaxes(title_text='Tramsmission (DAC Units)')
    fig.add_trace(go.Scatter(x=freqs, y=amps, mode='markers', name='data'))
    fig.add_trace(go.Scatter(x=freqs, y=y_fit, mode='lines', name='Fitted data'))
    fig.add_vline( x = f_r[0], line_width=1, line_dash="dash", line_color="Red")

    fitting_params = {'Resonance Freq (MHz)': f_r[0]}
    info_figure =  {
        'figure_json': fig.to_json(),
        'timestamp': time.time(),
        'fitting_params': fitting_params
    }
    response = send_figure('res_spec', info_figure)
    ...
    if choice == 1:
        ...
        print(f"\n Old res{QUBIT_INDEX} freq", config["res_freq_ge"])
        config_new["res_freq_ge"] = analysis_result["freq"]
        print(f"\n New res{QUBIT_INDEX} freq", config_new["res_freq_ge"])
        print("\n Resonator spec analysis finished...")
        print("===================================")
        print("saving to file...")
        with SlabFile('output_data.hdf5', 'a') as f:
            f.append_pt(expt_name, analysis_result_new["freq"])
