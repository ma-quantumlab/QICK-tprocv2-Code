...
"""
performs power rabi with a single shot experiment after
"""
while nodeio.status.active:
    ...
    rabi_config = {**config, **exp_config}

    # --- Acquires the data and runs the program --- #
    amp_rabi=AmplitudeRabiProgram(soccfg, reps=rabi_config['reps'], final_delay=rabi_config['relax_delay'], cfg=rabi_config)
    py_avg = rabi_config['py_avg']
    ...
    fit_result = fit.rabi(gains, ydata)
    ...
    with SlabFile(data_path + '\\' + fname, 'a') as f:
        ...
        del rabi_config['qubit_gain_ge']  # cant save QickParam
        # formats config into file as a single line
        f.attrs['config'] = json.dumps(rabi_config)
